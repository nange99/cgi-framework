...
expr_node *create_expr_node_for_variable(struct _context *c, char *variable)
{
	...
	o = template_get_variable(c, variable);
	...
	switch (o->type) {
	case CGI_STRING:
		tmp->value.str = o->value.u_str;
		tmp->type = STR;
		break;
	case CGI_INTEGER:
		tmp->value.lnum = o->value.u_int;
		tmp->type = LONG;
		break;
	case CGI_FLOAT:
		tmp->value.dnum = o->value.u_double;
		tmp->type = DOUBLE;
		break;
	default:
		tmp->type = -1;
...
int eval_op_equal(struct _context *c, expr_node *r, expr_node *l)
{
	...
	if ((r->type == BOOL || r->type == LONG) && (l->type == BOOL || l->type
	                == LONG)) {
